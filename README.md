...
```
 _____ _                 ____                     _      
|_   _(_)_ __  _   _    / ___|_      _____   ___ | | ___ 
  | | | | |_ \| | | |   \___ \ \ /\ / / _ \ / _ \| |/ _ \
  | | | | | | | |_| |    ___) \ V  V / (_) | (_) | |  __/
  |_| |_|_| |_|\____|   |____/ \_/\_/ \___/ \___/|_|\___|
               |___/
...
>> Tiny_Swoole_manager: 为 manager 进程<br />
>> Tiny_Swoole_task: N 个 task 进程 <br />
>> Tiny_Swoole_worker: M 个 worker 进程 <br />

#### UDP 服务
> 将 udp 段的enable 设置为 true, 其他服务设置为 false <br />
> sh shell/socket.sh restart 重启服务 <br />
> ps -ef | grep Tiny 将看到 <br />
>> Tiny_Swoole_udp_master: 为 master 进程  <br />
>> Tiny_Swoole_manager: 为 manager 进程<br />
>> Tiny_Swoole_task: N 个 task 进程 <br />
>> Tiny_Swoole_worker: M 个 worker 进程 <br />
...
```
> 4：自行维持心跳机制, 示例文件 client/ws.html# Swoole_nlp
